...
1. **Точные ширины колонок и разбивка широких строк** — RESOLVED (на усмотрение исполнителя, финал — приёмка D-07)
   ...
   - Принято: делать колонку «Поле» шире (≈6000) для длинных подписей, «Значение» уже; финально — глазами на приёмке (D-07).

2. **Оставлять ли первый пустой `<w:p/>` шаблона** — RESOLVED
   - Принято: заменить его первым осмысленным параграфом (заголовок блока 0 или вводную «Как заполнять»), чтобы бланк не начинался с пустой строки.
